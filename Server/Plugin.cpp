...
cell AMX_NATIVE_CALL GetVKeyStateProc(AMX* amx, cell* params)
{
	if ((Network::GetPlayerFromPlayerid(params[1])->GetNewkeys()) & params[2])
		return KEY_STATE_PRESSED;
	else
		return KEY_STATE_NOT_PRESSED;
}

cell AMX_NATIVE_CALL GetMousePosProc(AMX* amx, cell* params)
{
	cell* addr[2] = { NULL, NULL };
	amx_GetAddr(amx, params[2], &addr[0]);
	amx_GetAddr(amx, params[3], &addr[1]);

	Network::GetPlayerFromPlayerid(params[1])->GetMousePos(*addr[0], *addr[1]);
	return 1;
}
...
AMX_NATIVE_INFO PluginNatives[] =
{
	...
	{ "GetPlayerVirtualKeyState", GetVKeyStateProc },
	{ "GetPlayerMousePos", GetMousePosProc },
	{ 0, 0 }
};
