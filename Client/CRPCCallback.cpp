...
void CRPCCallback::Initialize()
{
	...
	CRPC::Add(eRPC::GET_VIRTUAL_KEY_STATE, GetVirtualKeyState);
	CRPC::Add(eRPC::GET_MOUSE_POS, GetMousePos);
...
RPC_CALLBACK CRPCCallback::GetVirtualKeyState(RakNet::BitStream& bsData, int iExtra)
{
	int vkey;
	if (bsData.Read(vkey))
	{
		CLocalPlayer::GetVKeyState(vkey);
	}
}

RPC_CALLBACK CRPCCallback::GetMousePos(RakNet::BitStream& bsData, int iExtra)
{
	int x, y;
	if (bsData.Read(x) && bsData.Read(y))
	{
		CLocalPlayer::GetMousePos(x, y);
	}
}